<template>
    <aside :class="asideClasses">
      <div class="sidebar-inner flex flex-col h-full">
        ...
        <div
          class="toggle-container absolute inset-x-0 top-1/2 transform -translate-y-1/2 -left-4 z-20"
        >
          <button
            class="toggle-btn"
            @click="toggleSidebar"
        ...
        <div v-if="!isCollapsed" class="flex flex-col flex-1 max-h-full">
          <!-- Header -->
          <div class="sidebar-header flex justify-between items-center p-2 pl-4">
            <div class="flex flex-1 items-center gap-2">
              <h2 class="text-med pr-2 font-semibold">
                Your Memories ({{ memories.length }})
              </h2>
            </div>
            <div class="flex justify-end gap-2 max-h-full">
              <button
                class="action-btn"
                @click="fetchMemories"
                :disabled="isLoading"
          ...
          <!-- Memory list -->
          <div class="memory-list max-h-full overflow-y-scroll" dir="ltr">
